...
public class Robot extends TimedRobot {
  ...
  public void robotPeriodic() {
    ...
    //SmartDashboard.putNumber("Gripper Position", Grip_Encoder.getPosition());
    SmartDashboard.putNumber("Winch Position", Winch_Encoder.getPosition()); 


    SmartDashboard.putNumber("Encoder Yaw", gyro.getAngle());
    SmartDashboard.putNumber("Encoder X complementary angle", gyro.getXComplementaryAngle()); 
    SmartDashboard.putNumber("Winch Position", gyro.getYComplementaryAngle()); 
...
//test
  ...
  public void autonomousPeriodic() {
    ...
    switch (m_autoSelected) {
      ...
      x = true;
      while(x == true){
        if(Grip_Encoder.getPosition() > -48) {
          m_handgripper.set(-0.4);
        }
        else {
          m_arm.set(0);
          x = false;
  ...
  /* if (m_driverController.getRightY() > 0.1) {
    ...
    double enc_RearLowest = 2.200;
    double enc_FrontLowest = -0.565;
    ...
    if (Math.abs(m_armController.getLeftY()) > 0.05){ //Accounts for slight stick drift by increasing deadzone by 5%
      if(m_armController.getLeftY() > 0 && enc_position > (enc_RearLowest - 0.15)) {
        slowArm = 0.1;
        DangerZone = true;
      }
      ...
      // a new speed and can be completely separate from the code for moving the arm if i'm not mistaken
      else if(m_armController.getLeftY() < 0 && enc_position < (enc_FrontLowest + 0.15)) {
        slowArm = 0.1;
        DangerZone = true;
      }
      else {
